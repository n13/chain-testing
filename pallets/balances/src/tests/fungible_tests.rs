...
fn unbalanced_trait_set_balance_works() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_ok!(<Balances as fungible::MutateHold<_>>::hold(
            &TestId::Foo,
            &1337,
            60
        ));
        ...
        assert_eq!(
            <Balances as fungible::InspectHold<_>>::total_balance_on_hold(&1337),
            60
        );
        assert_eq!(
...
fn unbalanced_trait_decrease_balance_simple_works() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_ok!(Balances::decrease_balance(
            &1337, 20, Exact, Expendable, Polite
        ));
...
fn unbalanced_trait_decrease_balance_works_1() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(
            Balances::decrease_balance(&1337, 100, Exact, Expendable, Polite),
            Ok(100)
        );
        assert_eq!(<Balances as fungible::Inspect<_>>::balance(&1337), 0);
    });
...
fn unbalanced_trait_decrease_balance_works_2() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(
            Balances::decrease_balance(&1337, 39, Exact, Expendable, Polite),
            Ok(39)
        );
        assert_eq!(<Balances as fungible::Inspect<_>>::balance(&1337), 1);
        assert_eq!(Balances::total_balance_on_hold(&1337), 60);
...
fn unbalanced_trait_decrease_balance_at_most_works_1() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(
            Balances::decrease_balance(&1337, 101, BestEffort, Expendable, Polite),
            Ok(100)
        );
        assert_eq!(<Balances as fungible::Inspect<_>>::balance(&1337), 0);
    });
...
fn unbalanced_trait_decrease_balance_at_most_works_2() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(
            Balances::decrease_balance(&1337, 99, BestEffort, Expendable, Polite),
            Ok(99)
        );
        assert_eq!(<Balances as fungible::Inspect<_>>::balance(&1337), 0);
    });
...
fn unbalanced_trait_decrease_balance_at_most_works_3() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(Balances::total_balance_on_hold(&1337), 60);
        assert_eq!(
            Balances::decrease_balance(&1337, 0, BestEffort, Expendable, Polite),
            Ok(0)
        );
        assert_eq!(Balances::free_balance(1337), 40);
        assert_eq!(Balances::total_balance_on_hold(&1337), 60);
        assert_eq!(
            Balances::decrease_balance(&1337, 10, BestEffort, Expendable, Polite),
            Ok(10)
        );
        assert_eq!(Balances::free_balance(1337), 30);
        assert_eq!(
            Balances::decrease_balance(&1337, 200, BestEffort, Expendable, Polite),
            Ok(29)
        );
        assert_eq!(<Balances as fungible::Inspect<_>>::balance(&1337), 1);
...
fn unbalanced_trait_increase_balance_works() {
    ExtBuilder::default().build_and_execute_with(|| {
        assert_noop!(
            Balances::increase_balance(&1337, 0, Exact),
            TokenError::BelowMinimum
        );
        assert_eq!(Balances::increase_balance(&1337, 1, Exact), Ok(1));
        assert_noop!(
            Balances::increase_balance(&1337, Balance::MAX, Exact),
...
fn unbalanced_trait_increase_balance_at_most_works() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_eq!(
            Balances::increase_balance(&1337, Balance::MAX, BestEffort),
            Ok(Balance::MAX - 1)
        );
    });
}
...
fn can_hold_entire_balance_when_second_provider() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_noop!(
                Balances::hold(&TestId::Foo, &1, 100),
                TokenError::FundsUnavailable
            );
            System::inc_providers(&1);
...
fn freeze_consideration_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                Balances::balance_frozen(&TestId::Foo, &who),
                4 + extend_freeze
            );
...
fn hold_consideration_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                Balances::balance_on_hold(&TestId::Foo, &who),
                4 + extend_hold
            );
...
fn lone_freeze_consideration_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                ticket.update(&who, Footprint::from_parts(0, 0)).unwrap(),
                zero_ticket
            );
            assert_eq!(Balances::balance_frozen(&TestId::Foo, &who), 0);
