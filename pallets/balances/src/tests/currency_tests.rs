...
fn inspect_lock_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                <Balances as InspectLockableCurrency<_>>::balance_locked(ID_1, &1),
                10
            );
            assert_eq!(
                <Balances as InspectLockableCurrency<_>>::balance_locked(ID_2, &1),
                10
            );
            assert_eq!(
                <Balances as InspectLockableCurrency<_>>::balance_locked(ID_1, &2),
                20
            );
            assert_eq!(
                <Balances as InspectLockableCurrency<_>>::balance_locked(ID_2, &2),
                0
            );
            assert_eq!(
                <Balances as InspectLockableCurrency<_>>::balance_locked(ID_1, &3),
                0
            );
        })
}
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                ...
                frame_system::IncRefStatus::Existed
            );
...
fn slash_over_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                Balances::slash(&1, 1_300),
                (NegativeImbalance::new(1000), 300)
            );
            // Account is dead
            assert!(!System::account_exists(&1));
        });
...
fn slash_full_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            assert_eq!(
                Balances::slash(&1, 1_000),
                (NegativeImbalance::new(1000), 0)
            );
...
fn slash_reserved_slash_partial_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            // Slashed completed in full
            assert_eq!(
                Balances::slash_reserved(&1, 800),
                (NegativeImbalance::new(800), 0)
            );
            assert_eq!(System::consumers(&1), 1);
            assert_eq!(Balances::reserved_balance(1), 100);
            assert_eq!(Balances::free_balance(1), 100);
...
fn slash_reserved_slash_everything_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            ...
            // Slashed completed in full
            assert_eq!(
                Balances::slash_reserved(&1, 900),
                (NegativeImbalance::new(900), 0)
            );
            assert_eq!(System::consumers(&1), 0);
            // Account is still alive
            assert!(System::account_exists(&1));
...
fn slash_reserved_on_non_existent_works() {
    ExtBuilder::default()
        ...
        .build_and_execute_with(|| {
            // Slash on non-existent account is okay.
            assert_eq!(
                Balances::slash_reserved(&12345, 1_300),
                (NegativeImbalance::new(0), 1300)
            );
        });
}
...
fn named_reserve_should_work() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_noop!(
            Balances::reserve_named(&id_3, &1, 2),
        ...
        assert_eq!(
            Balances::repatriate_reserved_named(&id_2, &1, &2, 10, Reserved).unwrap(),
            0
        );
        ...
        assert_eq!(
            Balances::repatriate_reserved_named(&id_2, &2, &1, 11, Reserved).unwrap(),
            1
        );
        ...
        assert_eq!(
            Balances::repatriate_reserved_named(&id_2, &1, &2, 10, Free).unwrap(),
            0
        );
        assert_eq!(Balances::reserved_balance_named(&id_2, &1), 5);
        assert_eq!(Balances::reserved_balance_named(&id_2, &2), 0);
        ...
        assert_eq!(
            Balances::repatriate_reserved_named(&id_2, &1, &1, 10, Reserved).unwrap(),
            5
        );
        assert_eq!(Balances::reserved_balance_named(&id_2, &1), 5);
        ...
        assert_eq!(
            Balances::repatriate_reserved_named(&id_2, &1, &1, 15, Free).unwrap(),
            10
        );
        assert_eq!(Balances::reserved_balance_named(&id_2, &1), 0);
...
fn reserved_named_to_yourself_should_work() {
    ExtBuilder::default().build_and_execute_with(|| {
        ...
        assert_ok!(
            Balances::repatriate_reserved_named(&id, &1, &1, 50, Free),
            0
        );
        assert_eq!(Balances::free_balance(1), 110);
        ...
        assert_ok!(Balances::reserve_named(&id, &1, 50));
        assert_ok!(
            Balances::repatriate_reserved_named(&id, &1, &1, 60, Free),
            10
        );
        assert_eq!(Balances::free_balance(1), 110);
        assert_eq!(Balances::reserved_balance_named(&id, &1), 0);
